...
const { MercadopagoProvider } = require("../dist/providers/index.js");
...
describe("Easy Pix", () => {
  ...
  describe("Mercado Pago", () => {
    ...
    test("Constructor creates instance with valid API key, and be type of Mmercado Pago", async () => {
      const easyPix = new EasyPix({
    ...
    test("Create method generates Pix and adds to pending payments", async () => {
      ...
      jest.spyOn(MercadopagoProvider.prototype, 'createPixPayment').mockResolvedValue({
        originalId: "mocked_original_id",
        value: 100,
        netValue: 98,
        encodedImage: "mocked_encoded_image",
      ...
      expect(MercadopagoProvider.prototype.createPixPayment).toHaveBeenCalledWith({
        taxId: "123456789",
        description: "Test Pix",
        id: "123",
        name: "John Doe",
    ...
    test("DeleteCob method removes payment from pending payments", async () => {
      jest.spyOn(MercadopagoProvider.prototype, 'deletePixChargeByPaymentId').mockResolvedValue({
        deleted: true,
      });
      ...
      expect(easyPix.pendingPayments).toEqual([]);
      expect(MercadopagoProvider.prototype.deletePixChargeByPaymentId).toHaveBeenCalledWith("mocked_original_id");
    
      jest.runOnlyPendingTimers();
    });

    test("Transfer method calls MercadoPagoRequests.createPixTransfer", async () => {
      jest.spyOn(MercadopagoProvider.prototype, 'createPixTransfer').mockResolvedValue({
        authorized: true,
        transferFee: 2,
        netValue: 98,
        value: 100,
    ...
    test("OnDue callback is called when payment is overdue", async () => {
      jest.spyOn(MercadopagoProvider.prototype, 'getPixPaymentStatusByPaymentId').mockResolvedValue("OVERDUE");
    ...
    test("OnPaid callback is called when payment is confirmed", async () => {
      jest.spyOn(MercadopagoProvider.prototype, 'getPixPaymentStatusByPaymentId').mockResolvedValue("CONFIRMED");
    ...
    test("#overdue method handles overdue payment correctly", async () => {
      ...
      jest.spyOn(MercadopagoProvider.prototype, 'deletePixChargeByPaymentId').mockResolvedValue({ deleted: true });
