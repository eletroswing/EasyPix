...
const MercadopagoRequests = require("./../dist/mercado-pago/index.js");
...
jest.mock("./../dist/mercado-pago/index.js");
jest.mock("axios");
...
describe("Easy Pix", () => {
  ...
  describe("Mercado Pago", () => {
    ...
    test("Constructor throws error when API key is missing", () => {
      expect(() => new EasyPix(undefined, true, 60, "MERCADOPAGO")).toThrow(
    ...
    test("Constructor creates instance with valid API key, and be type of mercado pago", async () => {
      const easyPix = new EasyPix("valid_api_key", true, 60, "MERCADOPAGO");
      expect(easyPix).toBeInstanceOf(EasyPix);
      expect(easyPix.provider).toBe("MERCADOPAGO");
      expect(easyPix.apiInterface).toBeInstanceOf(MercadopagoRequests);
      expect(easyPix.configPath).toBe("./");
      expect(easyPix.loopSecondsDelay).toBe(60);
      expect(easyPix.apiKey).toBe("valid_api_key");
    ...
    test("Create method generates Pix and adds to pending payments", async () => {
      MercadopagoRequests.prototype.generatePix.mockResolvedValue({
        encodedImage: "mocked_encoded_image",
        payload: "mocked_payload",
        expirationDate: new Date(),
        originalId: "mocked_original_id",
      ...
      expect(MercadopagoRequests.prototype.generatePix).toHaveBeenCalledWith({
        cpfCnpj: "123456789",
        description: "Test Pix",
        id: "123",
        name: "John Doe",
    ...
    test("DeleteCob method removes payment from pending payments", async () => {
      MercadopagoRequests.prototype.delPixCob.mockResolvedValue({
        deleted: true,
      });
      ...
      expect(easyPix.pendingPayments).toEqual([]);
      expect(MercadopagoRequests.prototype.delPixCob).toHaveBeenCalledWith(
        "mocked_original_id"
      );
    ...
    test("Transfer method calls MercadopagoRequests.transfer", async () => {
      MercadopagoRequests.prototype.transfer.mockRejectedValue(new Error('Not implemented by the gatway'));

      const easyPix = new EasyPix("valid_api_key", true, 60, "MERCADOPAGO");
      
      expect(easyPix.transfer()).rejects.toThrow('Not implemented by the gatway'); 

      expect(MercadopagoRequests.prototype.transfer).toHaveBeenCalled();

      jest.runOnlyPendingTimers();
    });

    test("OnDue callback is called when payment is overdue", async () => {
      MercadopagoRequests.prototype.getPixStatus.mockResolvedValue("OVERDUE");
    ...
    test("OnPaid callback is called when payment is confirmed", async () => {
      MercadopagoRequests.prototype.getPixStatus.mockResolvedValue("CONFIRMED");
    ...
    test("#overdue method handles overdue payment correctly", async () => {
      MercadopagoRequests.prototype.getPixStatus.mockResolvedValue("OVERDUE");
      MercadopagoRequests.prototype.delPixCob.mockResolvedValue(true);
